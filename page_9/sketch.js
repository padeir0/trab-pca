...
function drawArrow(base, end, color = 'black') {
...
function linearCloud(center, dir, numPoints) {
  let out = [];
  for (let i = 0; i < numPoints; i++) {
    let a = randomGaussian(0, 1);
    let offset = p5.Vector.mult(dir, a);
...
function generateClusters() {
  ...
  let dir = p5.Vector.sub(right, left).mult(0.15);

  {
    let numPoints = randint(minPointsPerCluster, maxPointsPerCluster);
    let points = linearCloud(left, dir, numPoints)
    clusters.push(new Cluster(points));
  }

  {
    let numPoints = randint(minPointsPerCluster, maxPointsPerCluster);
    let points = linearCloud(right, dir, numPoints)
    clusters.push(new Cluster(points));
  }
}
...
function setup() {
  ...
  document.addEventListener("keydown", e => {
    ...
    if (e.key === "ArrowLeft") location.assign("../page_8/index.html");
    if (e.key === "ArrowRight") location.assign("../page_10/index.html");
...
function draw() {
  ...
  drawArrow(clusters[0].findCentroid(), clusters[1].findCentroid());
